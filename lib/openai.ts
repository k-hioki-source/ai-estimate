...
写真・図面を参考に、元資料には存在しない製品断面図を新規に作成する場合は、
単純な写真トレースとして評価しないでください。
...
sourceType が reference_drawing で、「断面図」「断面を作成」「断面を作図」などが
依頼内容にある場合は、workType は原則 technical_drawing としてください。
「写真・図面を参考に」という記載があっても photo_trace / standard_trace に落とさないでください。

difficultyScore は原則65以上、structureComplexity は65以上を目安としてください。
想定工数は次を最低基準としてください。

・線画断面図：estimatedHours 6時間前後
・カラー断面図：estimatedHours 8時間前後
・リアル断面図：estimatedHours 12時間前後

内部部品が多い、複数断面が必要、引出し線・部品番号・名称ラベルが多い、
複雑な精密機構を含む場合は、この基準より工数を増やしてください。
単純な外観トレースの工数をそのまま適用しないでください。

【高密度な白黒線画の写真トレース】

写真トレースで、用途が取扱説明書、表現が白黒線画の場合でも、
画像内に実際に見えている線要素が非常に多い場合は高密度案件として評価してください。

重要：製品カテゴリから内部構造を推測しないでください。
画像に見えていないファン、基板、スロット、穴、内部部品などを想像して加算してはいけません。
『パソコン』『カメラ』『機械』という名称だけで高密度扱いにしないでください。
必ず画像上で確認できる線量・反復形状・細部の数を基準にします。

高密度の例：
・基板上に多数の電子部品やコネクターが実際に見えている
・多数の穴、スリット、フィン、配線、ハーネスが画面内に密集している
・複数ユニットが同時に描かれ、それぞれに細部が多い
・細かな輪郭線や内部線が画面全体に高密度で存在する
...
・partDensity：70以上
・lineDifficulty：70以上
・estimatedHours：5時間前後

一方、外形中心でボタンや表示窓など少数の要素しかなく、余白が多い単体製品は高密度ではありません。
例：体温計、リモコン、単純な工具、小型家電、シンプルな筐体など。
この場合は simple_trace を優先し、difficultyScore 20〜35、estimatedHours 0.8〜1.5時間程度を目安にしてください。

AIコメントには、画像で実際に確認できない部品名や構造を書かないでください。
...
通常の1点は estimatedHoursMin 24時間前後、estimatedHours 26時間前後、
estimatedHoursMax 30時間前後を基準にしてください。

難易度スコアが80〜90であっても、それだけを理由に40時間以上へ引き上げないでください。
30時間を超える工数は、分解図・断面・内部構造・多数の部品・複雑な背景など、
明確に追加工程が確認できる場合に限ってください。

EV車両と搭載バッテリー、バッテリー筐体、高電圧コネクター、複雑な電装部品など、
形状再構築と細部描写が必要な対象は工数を下げすぎない一方、
通常の単体案件では26時間前後を中心値として評価してください。
