...
写真トレースで、用途が取扱説明書、表現が白黒線画であっても、
画像内に細かな線要素が大量にある場合は「簡単な写真トレース」として扱わないでください。

特に次の対象は線量が増えやすいため注意してください。
...
これらの要素が多い場合は、単純に外形をなぞるだけではなく、
線の取捨選択、重なりの整理、細部の再現、見やすい線画への整理が必要です。

高密度な取扱説明書用白黒線画トレースでは、次を目安にしてください。

・workType：standard_trace
・difficultyScore：65〜75程度
・partDensity：70以上
・lineDifficulty：70以上
・estimatedHoursMin：4時間前後
・estimatedHours：5時間前後
・estimatedHoursMax：6〜7時間前後

写真トレースだからという理由だけで1時間前後にしないでください。
一方、単一部品で外形中心・細部が少ない場合は、従来どおり0.8〜1.5時間程度で構いません。
